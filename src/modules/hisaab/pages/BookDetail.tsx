...
import { ChevronLeft, MoreVertical, SlidersHorizontal } from 'lucide-react';
...
export default function BookDetail() {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [transactions, filter, singleDay, rangeFrom, rangeTo, categoryFilter]);
  ...
  return (
    <div className="h-screen bg-bg-primary flex flex-col">
      <header className="px-2 pt-12 pb-2 flex items-center gap-1 flex-shrink-0">
        ...
        <span className="flex-1 min-w-0 flex items-center gap-2 text-body text-hisaabText-primary">
          <span className="w-7 h-7 rounded-full bg-bg-icon flex items-center justify-center flex-shrink-0 text-hisaabText-secondary">
            <BookIcon iconId={book.emoji} size={14} />
          </span>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto pb-32">
        <div className="px-6 pt-3 pb-1">
          ...
          <AmountDisplay paise={balance} currency={book.currency} size="lg" className="mt-1 text-amount-lg" />
        </div>

        <div className="flex gap-2.5 px-5 pt-3">
          <div className="flex-1 bg-bg-card rounded-icon px-3.5 py-3 shadow-card">
            <p className="text-caption text-hisaabText-secondary">CASH IN</p>
            <p className={cn('font-sans tabular-nums text-body mt-1 whitespace-nowrap', statIn > 0 ? 'text-hisaabAccent-positive' : 'text-hisaabText-primary')}>{formatAmount(statIn, book.currency)}</p>
          </div>
          <div className="flex-1 bg-bg-card rounded-icon px-3.5 py-3 shadow-card">
            <p className="text-caption text-hisaabText-secondary">CASH OUT</p>
            <p className={cn('font-sans tabular-nums text-body mt-1 whitespace-nowrap', statOut > 0 ? 'text-hisaabAccent-negative' : 'text-hisaabText-primary')}>{formatAmount(statOut, book.currency)}</p>
          </div>
          <div className="flex-1 bg-bg-card rounded-icon px-3.5 py-3 shadow-card">
            <p className="text-caption text-hisaabText-secondary">ENTRIES</p>
            <p className="font-sans tabular-nums text-body text-hisaabText-primary mt-1">{transactions.length}</p>
          </div>
        </div>

        <div className="px-4 pt-4">
          <div className="flex items-center justify-between px-2 pb-2">
            <p className="text-caption text-hisaabText-secondary uppercase tracking-wide">Activity</p>
            <button
              onClick={openFilter}
              className={cn(
                'flex items-center gap-1.5 px-3 py-1.5 rounded-button text-caption transition-colors',
                isFiltered
                  ? 'bg-hisaabAccent-button text-hisaabAccent-buttonText'
                  : 'text-hisaabText-secondary active:bg-bg-hover',
              )}
            >
          ...
          {txLoading ? (
            ...
            <div className="bg-bg-card rounded-card shadow-card">
              <EmptyState
                ...
                description={transactions.length === 0 ? 'Tap Add Entry to record your first cash in or out.' : 'Try adjusting the filters.'}
              />
            </div>
          ) : (
            <div className="bg-bg-card rounded-card shadow-card overflow-hidden relative" style={{ height: virtualizer.getTotalSize() }}>
              {virtualizer.getVirtualItems().map((item) => {
                const tx = filtered[item.index];
                return (
                  <div
                    key={tx.id}
      ...
      <div className="fixed bottom-8 inset-x-4">
        <button
          ...
          className="w-full bg-hisaabAccent-button text-hisaabAccent-buttonText rounded-button py-4 text-body shadow-button active:scale-[0.98] transition-transform duration-100"
        >
          + Add Entry
        </button>
      </div>

      <BookOptionsSheet book={book} open={optionsOpen} onClose={() => setOptionsOpen(false)} />
