...
import { updateMemory } from '@/modules/nazara/db/memories';
import { savePhoto, deletePhoto } from '@/modules/nazara/db/photos';
import { usePhotoUrls } from '@/modules/nazara/shared/hooks/usePhotoUrl';
...
export default function EditMemory() {
  ...
  const existingUrls = usePhotoUrls(existingPhotoIds);

  useEffect(() => {
    if (memory && !initialized) {
      setTitle(memory.title);
      setDate(new Date(memory.date).toISOString().slice(0, 10));
      setType(memory.type);
      setNotes(memory.notes ?? '');
      setExistingPhotoIds(memory.photoIds);
      setPeople(memory.people);
      setTags(memory.tags ?? []);
      setCategory(memory.category);
      setNotifyYearly(memory.notifyYearly);
      setInitialized(true);
    }
  }, [memory, initialized]);
  ...
  const removeExistingPhoto = (pid: string) => {
    setExistingPhotoIds((p) => p.filter((id_) => id_ !== pid));
    setRemovedPhotoIds((p) => [...p, pid]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await updateMemory(memory.id, {
        ...
        photoIds: [...existingPhotoIds, ...uploadedIds],
        people,
        tags,
        category,
      ...
      removedPhotoIds.forEach((pid) => deletePhoto(pid).catch(console.error));

      navigate(`/nazara/memory/${memory.id}`);
    } catch (e) {
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {existingPhotoIds.map((pid, idx) => {
              const url = existingUrls[idx];
              if (!url) return null;
              return (
                <div key={pid} style={{ position: 'relative', width: 80, height: 80, borderRadius: 12, overflow: 'hidden' }}>
                  <img src={url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  <button
                    onClick={() => removeExistingPhoto(pid)}
                    style={{
