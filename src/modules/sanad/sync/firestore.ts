...
export async function syncOnLogin(uid: string) {
  const [docsSnap, pendingDeletes] = await Promise.all([
    getDocs(collection(fsdb, 'users', uid, 'sanadDocuments')),
    db.pendingDeletes.toArray(),
  ]);
  const deletedDocumentIds = new Set(pendingDeletes.filter((pd) => pd.kind === 'document').map((pd) => pd.targetId));
  const documents: DocumentRecord[] = docsSnap.docs
    .filter((d) => !deletedDocumentIds.has(d.id))
    .map((d) => ({ ...(d.data() as DocumentRecord), pendingSync: false }));
  if (documents.length) await db.documents.bulkPut(documents);

  await syncFilesOnLogin(uid, pendingDeletes);
}

async function syncFilesOnLogin(uid: string, pendingDeletes: { kind: string; targetId: string }[]) {
  const deletedFileIds = new Set(pendingDeletes.filter((pd) => pd.kind === 'file').map((pd) => pd.targetId));
  ...
  await Promise.all(
    filesSnap.docs.filter((d) => !deletedFileIds.has(d.id)).map(async (d) => {
      ...
      if (existingIds.has(id)) {
        await db.files.update(id, { pendingSync: false });
        return;
      }
      try {
        ...
        await db.files.put({ id, blob, thumbnail, mimeType, fileName, createdAt, pendingSync: false });
